...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  game_state_t *sharedGameState =
      createSharedMemory("/game_state", totalSize, O_CREAT | O_RDWR,
                         PERMISSIONS, PROTECTIONS, MAPFLAG);
  ...
  game_sync_t *sharedGameSync =
      createSharedMemory("/game_sync", sizeof(game_sync_t), O_CREAT | O_RDWR,
                         PERMISSIONS, PROTECTIONS, MAPFLAG);

  initalizeGameState(sharedGameState, width, height, players_count);

  printGameState(sharedGameState->board, sharedGameState->height,
                 sharedGameState->width, sharedGameState->players_count,
