...
static void run_master_loop(game_t *game, parameters_t *params, int32_t pipes[][pipe_ends], fd_set *masterSet, int32_t maxFd, pid_t view_pid, bool *has_view) {
    time_t last_valid_move = time(NULL);
    int8_t start_player = 0;

    while (!should_exit) {
        if (check_timeout(last_valid_move, params->timeout)) break;
        ...
        if (round.any_valid) last_valid_move = time(NULL);
        if (round.any_move && *has_view) sync_view_frame(game, view_pid, has_view);
        if (round.any_move) usleep(params->delay * 1000);
        if (!any_player_alive(game->state)) break;
    }
}

int main(int argc, char *argv[]) {
    setup_signals();
    parameters_t params = { .width = default_width, .height = default_heigh, .delay = default_delay, .timeout = default_timeout, .seed = time(NULL), .view_path = default_view_path };

    if (parse(argc, argv, &params) != success || params.players_count == 0) {
        fprintf(stderr, "Usage: master -w <width> -h <height> -p <player> [-p <player> ...] [-v <view>] [-d <delay>] [-t <timeout>] [-s <seed>]\n");
    ...
    game_end(&game);
    if (has_view) game_sync_notify_view(game.sync);
    close_active_pipes(pipes, game.state->players, params.players_count);

    for (int8_t i = 0; i < params.players_count; i++) waitpid(game.state->players[i].player_id, NULL, 0);
    if (has_view) waitpid(view_pid, NULL, 0);
