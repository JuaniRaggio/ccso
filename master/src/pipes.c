...
void forkPlayers(int pipes[][2], int playersCount, game_state_t *game_state){
    ...
    for (int i = 0; i < playersCount; i++) {
        ...
        if (pid == 0) { // HIJO i
            ...
            // Cada hijo debe escribir unicamente en un pipe -> Cerramos todos los demas write-end del hijo (de cada hijo)
...
void initFdSet(fd_set *masterSet, int pipes[][2], int playersCount, int *maxFd){
    ...
    *maxFd = 0;
    ...
    for (int i = 0; i < playersCount; i++) {
        FD_SET(pipes[i][READ], masterSet);  //Agrega el READ-END de todos los pipes a readfds (por eso necesita &readfds)

        if (pipes[i][READ] > *maxFd){
            *maxFd = pipes[i][READ]; //Si el fd del pipe actual es mayor que maxfd, actualizar. Asi nos quedamos con el mayor 
        }
    }
}